...
| Decision | Choice | Why |
|---|---|---|
| **Image model** | OpenAI `gpt-image-1` (default) **or** OpenRouter Unified Image API (default `openai/gpt-image-2`) | gpt-image-1 gives reference editing, strong adherence, and `input_fidelity`. OpenRouter (`--provider openrouter`) defaults to `openai/gpt-image-2` with one key and lets you swap in Gemini 3.1 Flash Image, Seedream 4.5, FLUX.2, etc. via `--model`. Same reference-guided flow via `input_references`. Transparency is handled uniformly downstream (rembg), so the model choice is about style/cost, not alpha. |
...
| **Background removal** | **`rembg`** (local, free, U²-Net) as a mandatory pass | Image models don't reliably emit transparency, so we generate on a plain flat background (see `TECH_SPRITE`) and segment the subject out with rembg — deterministic, no per-image cost. Runs on cut-out sprites; terrain tiles and soft overlays (fog/shine) skip it. `--bg always` (default) / `auto` / `never`. |
